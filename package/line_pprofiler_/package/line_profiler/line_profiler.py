...
def output_ranking_to_txt(ranking, caller_dir, file_name):
    ...
    for part_name, part in ranking.items():
        with io.open( join(caller_dir, file_name + "_" + part_name + ".txt"), \
...
def line_profiler(function, enable = True, file_name = None, \
                  blacklist = [], whitelist = [], \
                  default_blacklist = ["site-packages", "Anaconda"], \
    ...
    if output_ranking == True:
        output_ranking_to_txt(ranking, caller_dir, file_name)
    
    return everything_you_want_to_see, ranking